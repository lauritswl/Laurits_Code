...
```{r}
pacman::p_load(tidytext, tidyverse, dplyr)
library(tidyverse)
...
# Normalizing, setting the ID as ID (so that ID isn't considered as a predictor, and doing the PCA)
...
pca_rec_train <- juice(rec_PCA)
pca_rec_test <- bake(rec_PCA, new_data = test_empirical)

# Applying the model
real_f0 <- bf(Condition ~ 1 + .)

get_prior(real_f0, pca_rec_train, family = bernoulli)
